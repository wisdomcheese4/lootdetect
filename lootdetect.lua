...
local ACCENT_PX = 4         -- width of the decorative left-edge accent bar
...
windower.register_event('prerender', function()
    ...
    for i = #active, 1, -1 do
        ...
        if elapsed >= total_life then
            ...
            local icon_block_w = p.has_icon and (ICON_SIZE + PADDING) or 0
            local box_w = PADDING + icon_block_w + p.text_w + PADDING
            local box_h = math.max(ICON_SIZE, p.text_h) + PADDING * 2
            ...
            windower.prim.set_position(p.fill_name, box_x + BORDER_PX, box_y + BORDER_PX)
            windower.prim.set_size(p.fill_name, box_w - BORDER_PX * 2, box_h - BORDER_PX * 2)
            ...
            windower.prim.set_size(p.accent_name, ACCENT_PX, box_h)
            windower.prim.set_color(p.accent_name, border_alpha, style.accent[1], style.accent[2], style.accent[3])

            local content_x = box_x + PADDING + ACCENT_PX
            local content_y = box_y + (box_h - ICON_SIZE) / 2

            if p.has_icon then
                windower.prim.set_position(p.icon_name, content_x, content_y)
                windower.prim.set_color(p.icon_name, content_alpha, 255, 255, 255)
                content_x = content_x + ICON_SIZE + PADDING
            end

            windower.text.set_location(p.text_name, content_x, box_y + (box_h - FONT_SIZE) / 2 - 2)
