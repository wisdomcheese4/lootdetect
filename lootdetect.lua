...
-- ============================================================
-- Rarity-aware styling
--
-- NOTE: I could not verify the exact "Rare"/"Ex" flag bit values from
-- a trustworthy source for Windower's specific res.items.flags layout
-- (community docs confirm these flags exist conceptually in FFXI, but
-- not the precise bits Windower exposes them as). Rather than guess
-- and risk silently misclassifying every item, this uses commonly-
-- cited candidate bits, but treat this as a starting point to verify
-- with //lootdetect iteminfo on a real rare/Ex item and a real normal
-- item, then adjust RARE_BIT/EX_BIT below if the styling picks the
-- wrong items.
-- ============================================================
local RARE_BIT = 0x8000
local EX_BIT = 0x4000

local function rarity_style_for(item_id)
    local item = item_id ~= 0 and res.items[item_id]
    if not item or not item.flags then
        return nil
    end
    local flags = item.flags

    -- Confirmed via //lootdetect iteminfo on a real item: flags is a
    -- table keyed by human-readable Title Case names, e.g. "Exclusive",
    -- "Equippable", "No Delivery". "Rare" isn't confirmed yet (the test
    -- item wasn't Rare), but very likely follows the same pattern based
    -- on "Exclusive" and "Equippable" both being plain single-word keys.
    if type(flags) == 'table' then
        local rare = flags.Rare or flags.rare
        local ex = flags.Exclusive or flags.exclusive
        if rare then
            return 'gold'
        end
        if ex then
    ...
    if type(flags) == 'number' then
        if flags % (RARE_BIT * 2) >= RARE_BIT then
            return 'gold'
        end
        if flags % (EX_BIT * 2) >= EX_BIT then
            return 'royal'
        end
    end
...
local function show_item_popup(item_id, item_name)
    -- Duplicate stacking: if this exact item is already showing, just
    -- bump its count and refresh the timer instead of stacking a whole
    -- new panel underneath it - much cleaner during a farming burst.
    for _, p in ipairs(active) do
        if not p.destroyed and p.item_id == item_id then
            p.count = p.count + 1
            windower.text.set_text(p.text_name, ('%s x%d'):format(p.base_name, p.count))
            p.start_time = os.clock()  -- refresh the fade timer
            return
        end
    end
    ...
    p.item_id = item_id
    p.base_name = item_name
    p.count = 1
    p.style_name = rarity_style_for(item_id) or settings.style
...
windower.register_event('prerender', function()
    ...
    for i = #active, 1, -1 do
        ...
        if elapsed >= total_life then
            ...
            -- Subtle "pop" on entrance: grows from 85% to 100% size
            -- during the fade-in window, then settles at full size.
            local pop_scale = 1.0
            if elapsed < FADE_IN then
                pop_scale = 0.85 + 0.15 * (elapsed / FADE_IN)
            end

            local style = STYLES[p.style_name] or current_style()
            ...
            local icon_size_scaled = ICON_SIZE * pop_scale
            local icon_block_w = p.has_icon and (icon_size_scaled + ICON_PAD * 2 + PADDING) or 0
            local box_w = (PADDING + icon_block_w + p.text_w + PADDING) * pop_scale
            local box_h = (math.max(ICON_SIZE + ICON_PAD * 2, p.text_h) + PADDING * 2 + TOP_STRIP_PX) * pop_scale
            ...
            local content_y = box_y + TOP_STRIP_PX + (box_h - TOP_STRIP_PX - icon_size_scaled - ICON_PAD * 2) / 2
            ...
            if p.has_icon then
                ...
                windower.prim.set_size(p.icon_bg_name, icon_size_scaled + ICON_PAD * 2, icon_size_scaled + ICON_PAD * 2)
                ...
                windower.prim.set_size(p.icon_name, icon_size_scaled, icon_size_scaled)
                windower.prim.set_color(p.icon_name, content_alpha, 255, 255, 255)
                content_x = content_x + icon_size_scaled + ICON_PAD * 2 + PADDING
            end
...
windower.register_event('addon command', function(...)
    ...
    elseif args[1] == 'iteminfo' then
        local name_parts = {}
        for i = 2, #args do
            table.insert(name_parts, args[i])
        end
        local query = table.concat(name_parts, ' ')
        build_name_cache()
        local id = item_id_by_name[query] or item_id_by_name[normalize_item_name(query)]
        if not id or not res.items[id] then
            windower.add_to_chat(207, 'LootDetect: could not resolve an item named "' .. query .. '".')
        else
            windower.add_to_chat(207, ('LootDetect: fields for "%s" (id %d):'):format(query, id))
            for k, v in pairs(res.items[id]) do
                if k == 'flags' and type(v) == 'table' then
                    windower.add_to_chat(207, '  flags = {')
                    for fk, fv in pairs(v) do
                        windower.add_to_chat(207, ('    %s = %s'):format(tostring(fk), tostring(fv)))
                    end
                    windower.add_to_chat(207, '  }')
                elseif k == 'flags' and type(v) == 'number' then
                    windower.add_to_chat(207, ('  flags = %d (0x%X)'):format(v, v))
                elseif type(v) ~= 'table' then
                    windower.add_to_chat(207, ('  %s = %s'):format(tostring(k), tostring(v)))
                end
            end
        end
